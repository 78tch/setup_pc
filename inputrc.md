...
6. ちなみに、それ以外のキーに設定したい場合に、どのように指定したらいいかを確認するには、cat コマンドを使います。  <kbd>cat</kbd>と打ってEnter、設定したいキーを押下。例では上下左右と矢印キーをを押下しています。「^[」を除いた部分を指定時に使います。調べ終わったら、<kbd>Ctrl + c </kbd>で抜けられます。
```sh
$ cat  
^[[A^[[B^[[D^[[C
```  
